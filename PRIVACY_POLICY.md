...
2. DOI identifiers are checked with both CrossRef and OpenAlex; PubMed IDs are checked with OpenAlex
...
4. Reference text and explicitly marked titles may be read to identify and display each bibliography entry. Titles may be compared locally against the published record; reference text and titles are not transmitted anywhere
...
- **OpenAlex API** (api.openalex.org) - To verify DOIs alongside CrossRef, look up PubMed IDs, and check its retraction flag
...
- Each cached entry is treated as expired after 24 hours and is removed on its next access or when the extension starts
- The cache contains normalized DOI/PubMed ID keys and public lookup results from the services above. Reference text, page titles, browsing URLs, and locally computed title comparisons are not cached
